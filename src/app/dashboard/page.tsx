      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          <div className="text-3xl font-bold text-indigo-600 dark:text-indigo-400">
            {widgets?.poSummary
              ? (widgets.poSummary["SUBMITTED"] ?? 0) + 
                (widgets.poSummary["APPROVED"] ?? 0) + 
                (widgets.poSummary["PARTIALLY_RECEIVED"] ?? 0)
              : 0}
          </div>
          <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">
            {widgets?.poSummary?.["SUBMITTED"] ?? 0} submitted ·{" "}
            {widgets?.poSummary?.["APPROVED"] ?? 0} approved
          </div>
        </div>
              ...
              <>
                <div className="relative w-28 h-28 mx-auto mb-4">
                  <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                    ...
                    <circle
                      ...
                      fill="none"
                      strokeWidth="3"
                      strokeDasharray={`${widgets.inventoryHealth.healthPercent} ${100 - widgets.inventoryHealth.healthPercent}`}
                      strokeLinecap="round"
                      className={
                        widgets.inventoryHealth.healthPercent >= 80
                          ? "text-green-500 stroke-current"
                          : widgets.inventoryHealth.healthPercent >= 50
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-xl font-bold text-gray-900 dark:text-gray-100">
                      {widgets.inventoryHealth.healthPercent}%
                    </span>
                  </div>
                </div>
                <div className="space-y-1 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-700 dark:text-gray-300">{widgets.inventoryHealth.healthy}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500 dark:text-gray-400">
                      <span className="inline-block w-2 h-2 rounded-full bg-yellow-500 mr-1" /> Low
                    </span>
                    <span className="font-medium text-gray-700 dark:text-gray-300">{widgets.inventoryHealth.low}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500 dark:text-gray-400">
                      <span className="inline-block w-2 h-2 rounded-full bg-red-500 mr-1" /> Critical
                    </span>
                    <span className="font-medium text-gray-700 dark:text-gray-300">{widgets.inventoryHealth.critical}</span>
                  </div>
                </div>
              </>
            ) : (
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          {widgets?.stockVelocity ? (
            <>
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-gray-500 dark:text-gray-400">Inbound</span>
                    <span className="font-medium text-green-600 dark:text-green-400">{widgets.stockVelocity.totalInbound}</span>
                  </div>
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div
                      className="h-full rounded-full bg-green-500 transition-all"
                      style={{ width: `${Math.min((widgets.stockVelocity.totalInbound / Math.max(widgets.stockVelocity.totalInbound, widgets.stockVelocity.totalOutbound, 1)) * 100, 100)}%`, minWidth: widgets.stockVelocity.totalInbound > 0 ? "4px" : "0" }}
                    />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="font-medium text-orange-600 dark:text-orange-400">{widgets.stockVelocity.totalOutbound}</span>
                  </div>
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div
                      className="h-full rounded-full bg-orange-400 transition-all"
                      style={{ width: `${Math.min((widgets.stockVelocity.totalOutbound / Math.max(widgets.stockVelocity.totalInbound, widgets.stockVelocity.totalOutbound, 1)) * 100, 100)}%`, minWidth: widgets.stockVelocity.totalOutbound > 0 ? "4px" : "0" }}
                    />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="font-medium text-blue-600 dark:text-blue-400">{widgets.stockVelocity.totalTransfers}</span>
                  </div>
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div
                      className="h-full rounded-full bg-blue-500 transition-all"
                      style={{ width: `${Math.min((widgets.stockVelocity.totalTransfers / Math.max(widgets.stockVelocity.totalInbound, widgets.stockVelocity.totalOutbound, 1)) * 100, 100)}%`, minWidth: widgets.stockVelocity.totalTransfers > 0 ? "4px" : "0" }}
                    />
                  </div>
                </div>
              </div>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-4 text-center">Last {widgets.stockVelocity.periodDays} days</p>
            </>
          ) : (
            <p className="text-sm text-gray-400 text-center py-8">Loading...</p>
          )}
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          {widgets?.topMovers && widgets.topMovers.length > 0 ? (
            <div className="space-y-3">
              {widgets.topMovers.slice(0, 5).map((item, i) => (
                <div key={item.id} className="flex items-center gap-2">
                  <span className="text-xs font-bold text-gray-400 dark:text-gray-500 w-4 text-right">{i + 1}</span>
                  <div className="flex-1 min-w-0">
                    <button
                      onClick={() => router.push(`/dashboard/materials/${item.id}`)}
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            {widgets?.poSummary && Object.keys(widgets.poSummary).length > 0 ? (
              <div className="space-y-2">
                {Object.entries(widgets.poSummary).map(([status, count]) => {
                  const total = Object.values(widgets.poSummary).reduce((a, b) => a + b, 0);
