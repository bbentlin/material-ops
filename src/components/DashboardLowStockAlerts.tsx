"use client";

import type { LowStockAlert } from "@/types/dashboard";

type Props = {
  lowStockAlerts: LowStockAlert[];
  showAlerts: boolean;
  criticalCount: number;
  canEdit: boolean;
  onToggleShowAlertsAction: () => void;
  onOpenMaterialAction: (materialId: string) => void;
  onRestockAction: (materialId: string) => void;
};

export default function DashboardLowStockAlerts(props: Props) {
  if (props.lowStockAlerts.length === 0) return null;

  return (
    <div className="mb-8 overflow-hidden rounded-xl border border-orange-200 bg-white shadow-sm dark:border-orange-900/50 dark:bg-gray-800">
      <button
        onClick={props.onToggleShowAlertsAction}
        className="w-full p-4 transition-colors hover:bg-orange-50/50 dark:hover:bg-orange-900/10"
      >
        <div className="flex items-start justify-between gap-3 sm:items-center">
          <div className="flex min-w-0 flex-wrap items-center gap-2 sm:gap-3">
            <span className="text-lg">⚠️</span>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Low Stock Alerts</h3>

            <span className="rounded-full bg-orange-100 px-2 py-0.5 text-xs font-medium text-orange-700 dark:bg-orange-900 dark:text-orange-300">
              {props.lowStockAlerts.length} item{props.lowStockAlerts.length !== 1 ? "s" : ""}
            </span>

            {props.criticalCount > 0 && (
              <span className="rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700 dark:bg-red-900 dark:text-red-300">
                {props.criticalCount} critical
              </span>
            )}
          </div>

          <span className="shrink-0 text-sm text-gray-400">{props.showAlerts ? "▲ Hide" : "▼ Show"}</span>
        </div>
      </button>

      {props.showAlerts && (
        <div className="border-t border-orange-100 dark:border-orange-900/30">
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {props.lowStockAlerts.map((alert) => {
              const percent = Math.max(0, Math.min(alert.percentOfThreshold, 100));

              return (
                <div
                  key={alert.id}
                  className={`p-4 sm:px-5 sm:py-3 ${alert.severity === "CRITICAL" ? "bg-red-50/50 dark:bg-red-900/10" : ""}`}
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                    <div className="flex items-start gap-3 sm:items-center">
                      <span
                        className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-bold ${
                          alert.severity === "CRITICAL"
                            ? "bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300"
                            : "bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300"
                        }`}
                      >
                        {alert.severity === "CRITICAL" ? "CRITICAL" : "LOW"}
                      </span>

                      <div className="min-w-0">
                        <button
                          onClick={() => props.onOpenMaterialAction(alert.id)}
                          className="block truncate text-left text-sm font-medium text-blue-600 hover:text-blue-500 hover:underline"
                        >
                          {alert.name}
                        </button>
                        <span className="font-mono text-xs text-gray-400">{alert.partNumber}</span>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 gap-3 sm:ml-auto sm:grid-cols-[8rem_auto_auto_auto] sm:items-center sm:gap-4">
                      <div className="w-full sm:w-32">
                        <div className="mb-0.5 flex items-center justify-between text-xs">
                          <span
                            className={`font-bold ${
                              alert.severity === "CRITICAL"
                                ? "text-red-600 dark:text-red-400"
                                : "text-orange-600 dark:text-orange-400"
                            }`}
                          >
                            {alert.quantity}
                          </span>
                          <span className="text-gray-400">
                            / {alert.minQuantity} {alert.unit}
                          </span>
                        </div>
                        <div className="h-1.5 w-full rounded-full bg-gray-200 dark:bg-gray-700">
                          <div
                            className={`h-full rounded-full transition-all ${
                              alert.severity === "CRITICAL" ? "bg-red-500" : "bg-orange-400"
                            }`}
                            style={{ width: `${percent}%`, minWidth: alert.quantity > 0 ? "2px" : "0" }}
                          />
                        </div>
                      </div>

                      <span className="text-xs text-gray-500 dark:text-gray-400 sm:w-24 sm:text-right">
                        Need {alert.deficit} more
                      </span>

                      <span className="sm:w-28 sm:text-right">
                        {alert.department ? (
                          <span
                            className="inline-block rounded-full px-2 py-0.5 text-xs font-semibold text-white"
                            style={{ backgroundColor: alert.department.color }}
                          >
                            {alert.department.name}
                          </span>
                        ) : (
                          <span className="text-xs text-gray-400">-</span>
                        )}
                      </span>

                      {props.canEdit && (
                        <button
                          onClick={() => props.onRestockAction(alert.id)}
                          className="rounded-lg bg-green-600 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-green-700 sm:justify-self-end"
                        >
                          + Restock
                        </button>
                      )}
                    </div>
                  </div>
                </div>
              );
            })}
          </div>
        </div>
      )}
    </div>
  );
}